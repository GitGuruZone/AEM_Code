...
public class WorkflowExcel implements WorkflowProcess {
    ...
    private final Logger log
            = LoggerFactory.getLogger(WorkflowExcel.class);
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(path)) {
                ...
                InputStream inputStream
                        = rendition.adaptTo(InputStream.class);
                if (inputStream == null) {
                    log.error("Unable to adapt", excelResource.getPath());
                    return;
                }
                ...
                while (rowIterator.hasNext()) {
                    ...
                    if (name != null && roll != null) {
                        ...
                        Resource productResource
                                = resourceResolver.getResource(nodePath);
                        if (productResource == null) {
                            productResource = resourceResolver.create(
                                    resourceResolver.getResource(
                                            "/content"), name, null);
                        }

                        ModifiableValueMap properties
                          = productResource.adaptTo(ModifiableValueMap.class);
